...
- When the live bridge addon is enabled, `enable_live_bridge` and `get_live_bridge_status` also prepare a reusable session file so a normal Play run launched from the Godot editor can connect back to the MCP.
...
| `enable_live_bridge` | Enables the editor plugin metadata, adds the runtime autoload entry, and prepares a reusable live-bridge session file for editor-launched Play runs. |
...
If the addon is enabled, calling `get_live_bridge_status` also prepares the current MCP session for a normal Play run from the Godot editor. After that, you can press Play in the editor and poll `get_live_bridge_status` until the state changes to `connected_ready`.
